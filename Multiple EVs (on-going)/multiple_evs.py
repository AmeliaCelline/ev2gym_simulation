...
for cs in env.charging_stations:
    ...
    for port in range(cs.n_ports):
        df_EV_SOC[f'cs_{cs.id}'] = (env.port_energy_level[port, cs.id, :]) * 100 # convert to percentage

df['solar_power'] = env.tr_solar_power[0, :] * -1
df ['inflexible_load'] = env.tr_inflexible_loads[0, :]
df["carbon neutrality"] = df['solar_power'] - df ['inflexible_load'] - df['total_cs_power']
